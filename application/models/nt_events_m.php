<?php
class Nt_events_m extends DIP_Model{
	public $rules = array(
			'event_date' =>  array(
					'field'=>'event_date',
					'label'=>'Event Date',
	...
	public function save($post,$pid,$id=null){
		...
		// return $r;
		$success = $r->save();
		//redirect(site_full_url('golfclub/nexttee/nt_events'));
		if(! $success)
		{
			...
			// return $r->id;
			$eid = $r->id;
			$up_dir = 'uploads/'.$pid.'/nt_events/'.$eid.'/';
			...
			$a->get_by_id($eid);
				...
				if(isset($post['detail_post'])){
					...
					//$a->file_detail = $arrayNew;
					$a->file_detail = encode_data($arrayNew);
					//$abc = decode_data($a->file_detail);
					//print_r($abc);
					//die(ok);
				}
	...
	public function delete($pid,$id){
		...
		if($r->exists()){
			//delete imge folder
			$upload_url = 'uploads/'.$pid.'/nt_events/'.$id;
	...
	public function delete_all($pid){
		...
		if($r->exists()){
			$r->delete_all();
			$upload_url = 'uploads/'.$pid.'/nt_events';
				if (is_dir($upload_url))
					deleteFolder($upload_url);
			return TRUE;
		}else {
