<?php if(!defined('BASEPATH')) exit('No direct script access allowed');
  ...
  */
  class Jqgridg
  {
    ...
    */
    public function searchable($columns, $backtick_protect = TRUE)
    {
      foreach($this->explode(',', $columns) as $val)
      {
        $column = trim(preg_replace('/(.*)\s+as\s+(\w*)/i', '$2', $val));
        $column = preg_replace('/.*\.(.*)/i', '$1', $column); // get name after `.`
        $this->searchable_columns[] =  $column;
      }
      return $this;
    }
    ...
    */
    private function get_paging()
    {
      ...
      $iStart = $limit*$page - $limit;
      $iLength = $this->ci->input->get('rows');
    ...
    */
    private function get_ordering()
    {
      ...
      $Data = $this->ci->input->get('sidx');

      if ($this->ci->input->get('sord')){
        $this->ci->db->order_by($this->ci->input->get('sidx'), $this->ci->input->get('sord'));
      }

      // if ($this->ci->input->get('sord'))
      //   foreach ($this->ci->input->get('sord') as $key)
      //     if($this->check_cType())
    ...
    */
    ...
    {
      if(empty($this->ci->input->get('globalSearch')))
        return;

      $mColArray = $this->columns;

      $sWhere = '';
      $search = $this->ci->input->get('globalSearch');

      //this is the string
      $sSearch = $this->ci->db->escape_like_str(trim($search));
    ...
    */
    private function produce_output($output, $charset)
    {
      ...
      if($output == 'json')
      {
        ...
        $limit = ($this->ci->input->get('rows')?$this->ci->input->get('rows'):1);
        $page = ($this->ci->input->get('page')?$this->ci->input->get('page'):1);
...
/* End of file jqgrid.php */
/* Location: ./application/libraries/jqgrid.php */
