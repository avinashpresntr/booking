            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                if ($pkg->id && !isset($post['draft'])) {
                    ...
                    if (isset($post['push']) && ($total_push > $used_push)) {
                        ...
                        $this->load->library('apn_nt');
                        $this->apn_nt->payloadMethod = 'enhance';
                        $found = false;
                        $counter = false;
                                ...
                                if (isset($recipent) && $recipent->exists()) {
                                    ...
                                    foreach ($recipent as $recp) {
                                        ...
                                        if ($recp->os == 'ios') {
                                            ...
                                            $this->apn_nt->setData($msgIos); // aditional data
                                            $this->apn_nt->sendMessage($recp->token, $value, $recp->push_count);
                                        } else {
                                            $this->gcm->addRecepient($recp->token);
                                        }
                                    }
                                    $this->apn_nt->disconnectPush();
