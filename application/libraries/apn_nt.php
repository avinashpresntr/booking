...
/*******************************
	...
	*/
	...
	/**
	 * Prepares path to current pem file
	 *
	 * @param <array> $params
	 */
	protected function buildPermissionPath($params)
	{
		//modified by Andrey
		if($params['isSandbox'])
		{
			$pemfile = $params['PermissionDir'] . $params['PermissionFile'] . "_dev.pem";
		}
		else
		{
			$pemfile = $params['PermissionDir'] . $params['PermissionFile'] . "_prod.pem";
		}
...
*******************************/
	...
	function __construct($params) {
		...
		//prepare pem filepath and filename
		$params['PermissionDir'] = $this->_ci->config->item('PermissionDir','apn');
		$params['isSandbox'] = $this->_ci->config->item('Sandbox','apn'); //added by Andrey
		$this->permissionFile = $this->buildPermissionPath($params);


  // $permissionFile is set as a dynamic config when Apn library is loading.
	// for example in packages or news
	if(!file_exists($this->permissionFile))
	{
			//show_error("APN Failed to connect: APN Permission file not found: ");
			$this->permissionFile = $params['PermissionDir']."default.pem";
	}
		...
		//get certificate file name from params but not config file
		$this->keyCertFilePath = $this->permissionFile;
