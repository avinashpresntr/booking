<section class="dip-dash-sec">
  ...
  <?php echo form_open_multipart(current_full_url(), 'class="dip-form form-horizontal"');?>
        ...
        <?php foreach ($client->parent->languages as $key => $value):?>
          <div class="col-sm-4">
            ...
            <div class="dip-langbox <?=$default;?>">
              ...
              <?php echo form_textarea(array('name'=>'descr['.$value.']','rows'=>'2'), set_value('descr['.$value.']',$r_descr), 'class="form-control" rel="'.$value.'" id="dipDesc'.$value.'" placeholder="'.$this->lang->line('Description').'" '.$required);?>
    ...
    <div class="dip-form-foot text-center">
      <?php //echo form_submit('draft', 'Save as Draft','class="btn btn-warning"'); ?>
      <?php echo form_submit('submit', $this->lang->line('SavePublish'),'class="btn btn-success"'); ?>&nbsp;
      <a href="<?php echo site_full_url('partner/packages');?>" class="btn btn-default" ><?php echo $this->lang->line('Cancel'); ?></a>
    </div>
  <!-- </form> -->
  <?php echo form_close();?>
</section>
