		...
		<!-- load previous images -->
			<?php foreach ($pics as $key => $image):?>
				...
				style="width: 100%;"  data-image="<?php echo site_url($image['url']);?>">
				    <input id="dz-input-<?php echo $conut_dz;?>" class="drop" type="file" name="thumb2[<?php echo $conut_dz; ?>]"/>
				</div>
			<?php echo form_checkbox('previmage2['.$conut_dz.']', '1', false, 'id="dz2-check-'.$conut_dz.'" style="display:none;"'); ?>
		...
		<?php else:?>
			...
			data-width="<?php echo $config['width'];?>" data-height="<?php echo $config['height'];?>"  
			style="width: 100%;">
			    <input id="dz-input-<?php echo $conut_dz;?>" class="drop" type="file" name="thumb2[<?php echo $conut_dz; ?>]" <?php if(!isset($config['notrequired'])){echo 'required';}?>/>
			</div>
		<?php endif;?>
